def calculate_subnests(parsed_df, price_per_kg, cutting_price_per_sec):
    """
    Adds calculated fields to the Sub Nests DataFrame.
    Args:
        parsed_df (pd.DataFrame): DataFrame containing parsed sub nest data.
        price_per_kg (float): Price of material per kilogram.
    ...
    parsed_df["Total Material Price (€)"] = parsed_df["Total Weight (kg)"] * price_per_kg
...
def calculate_parts(parts_df, price_per_kg, cutting_price_per_sec):
    """
    ...
    parts_df["Price per Part (€)"] = (
        parts_df["Weight (kg)"] * price_per_kg
    ) + (
